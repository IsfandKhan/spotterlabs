...
export const api = {
  planTrip: (data: PlanRequest) =>
    request<TripPlan>("/api/trips", { method: "POST", body: JSON.stringify(data) }),
  getTrip: (id: number | string) => request<TripPlan>(`/api/trips/${id}`),
  listTrips: (page = 1) =>
    request<{ count: number; next: string | null; previous: string | null; results: TripListRow[] }>(
      `/api/trips?page=${page}`,
    ),
}
